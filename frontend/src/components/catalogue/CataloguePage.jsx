      ...
      <PopoverTrigger asChild>
        <Button
          ...
          className={cn(
            ...
            hasSelection &&
              "border-solid border-primary/50 bg-primary/5 text-primary",
            className,
          )}
        >
...
export function CataloguePage({
  ...
  const [selectedAdvertisingMark, setSelectedAdvertisingMark] = useState([
    "all",
  ]);
  const [showFavoritesOnly, setShowFavoritesOnly] =
    useState(initialShowFavorites);
  ...
  // This effect ensures the view updates if the prop changes

  const getQuantity = (id) => items.find((i) => i.id === id)?.quantity || 0;
  useEffect(() => {
    const getWebsites = async () => {
      try {
        ...
        const websitesArray =
          response && response.data && Array.isArray(response.data.websites)
            ? response.data.websites
            : [];

        const activeWebsites = websitesArray.filter(
          (web) => web && web.status === "active",
        );
        ...
        const formatted = activeWebsites.map((web, index) => {
          ...
          const siteCountry =
            web.countries && web.countries.length > 0
              ? web.countries[0]
              : "United States";
          const siteLanguage =
            web.language && web.language.length > 0
              ? web.language[0]
              : "English";
        ...
        console.error(
          "Database fetch failed, loading fallback mock data",
          error,
        );
        setLoading(false);
      }
  ...
  // Now filter your data based on showFavoritesOnly
  const displayedWebsites = showFavoritesOnly
    ? catalogueData.filter((site) => isFavorite(site.id))
    : catalogueData;
  const [selectedHomepageAnnouncements, setSelectedHomepageAnnouncements] =
    useState(["all"]);
  if (searchQuery) {
  ...
  if (selectedCountries.length > 0 && !selectedCountries.includes("all")) {
    ...
    filteredData = filteredData.filter((item) =>
      selectedCountries.some(
        (sel) => item.country.toLowerCase() === (countryMap[sel] ?? sel),
      ),
    );
  }
  ...
  if (selectedLanguages.length > 0 && !selectedLanguages.includes("all")) {
    ...
    filteredData = filteredData.filter((item) =>
      selectedLanguages.some(
        (sel) => item.language.toLowerCase() === (langMap[sel] ?? sel),
      ),
    );
  }
  ...
  if (selectedCopywriting.length > 0 && !selectedCopywriting.includes("all")) {
    if (
      selectedCopywriting.includes("yes") &&
      !selectedCopywriting.includes("no")
    ) {
  ...
  const handleAddToCart = (item) => {
    ...
    addItem(item); // This automatically handles incrementing the quantity
  ...
  const incrementCartQuantity = (item, e) => {
    e.stopPropagation();

    // 1. Update global store (Header cart)
    addItem(item);

    // 2. Update local state (The "1", "2" badge on the button)
    setCartItems((prev) => ({
      ...prev,
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-xs text-muted-foreground">
              Find the best websites for your marketing strategy
            </p>
          </div>
          <Button
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-3">
                  ...
                  <Button
                    ...
                    className={cn(
                      ...
                      showDiscountedOnly &&
                        "border-solid border-green-500/50 bg-green-500/5 text-green-600 hover:bg-green-500/10",
                    )}
                  >
                    <div className="flex items-center gap-2 truncate">
                    ...
                    {showDiscountedOnly && (
                      <CheckIcon className="h-3 w-3 shrink-0 opacity-50" />
                    )}
                  </Button>
                </div>
                  ...
                  <div className="pt-4 border-t border-dashed border-border space-y-6 animate-in slide-in-from-top-2 duration-200">
                        ...
                        <div className="relative min-h-[36px] bg-background border border-input rounded-md flex flex-wrap items-center gap-1.5 px-2.5 py-1 focus-within:ring-1 focus-within:ring-ring focus-within:border-primary transition-all">
                          ...
                          {keywords.map((keyword, index) => (
                            <Badge
                              key={index}
                              variant="secondary"
                              className="h-5 px-1.5 text-[10px] gap-1 font-normal bg-primary/10 text-primary hover:bg-primary/20 border-primary/20"
                            >
                              {keyword}
                              <XIcon
                          ...
                          {keywords.length < 3 && (
                            <input
                              ...
                              placeholder={
                                keywords.length === 0 ? "Type keyword..." : ""
                              }
                    ...
                    <div className="space-y-3">
                      <div className="flex items-center gap-2">
                        ...
                        <h4 className="text-xs font-bold text-foreground uppercase tracking-wider bg-muted/50 px-2 py-0.5 rounded">
                          Ahrefs Metrics
                        </h4>
                        <div className="h-px flex-1 bg-border"></div>
                      </div>
          ...
          <div className="overflow-x-auto">
            <table className="min-w-full divide-y divide-border">
              <thead className="bg-slate-50">
                <tr>
                  <th className="px-4 py-3 text-left text-[11px] font-semibold text-muted-foreground uppercase tracking-wider w-[240px]">
                    Website & Rating
                  </th>
                  <th className="px-4 py-3 text-left text-[11px] font-semibold text-muted-foreground uppercase tracking-wider w-[220px]">
                    Category & Topics
                  </th>
                  <th className="px-4 py-3 text-left text-[11px] font-semibold text-muted-foreground uppercase tracking-wider w-[180px]">
                    Region
                  </th>
                  ...
                  <th className="px-4 py-3 text-right text-[11px] font-semibold text-muted-foreground uppercase tracking-wider w-[120px]">
                    Pricing
                  </th>
                  <th className="px-4 py-3 text-right text-[11px] font-semibold text-muted-foreground uppercase tracking-wider w-[100px]">
                    Action
                  </th>
                </tr>
              </thead>
                          ...
                          <div className="flex items-start gap-2.5">
                            ...
                            <div className="flex flex-col gap-2 min-w-0">
                              <div className="flex items-center gap-1.5 flex-wrap">
                                ...
                                {isFavorite(item.id) && (
                                  <HeartIcon className="w-2.5 h-2.5 text-rose-500 fill-rose-500 shrink-0" />
                                )}
                              </div>
                                    ...
                                    <Tooltip>
                                      ...
                                      <TooltipContent className="max-w-[220px] bg-slate-900 text-white shadow-lg">
                                        ...
                                        <p className="text-xs leading-relaxed">
                                          Experienced and trusted publisher with
                                  ...
                                  {item.badges.fastDelivery && (
                                    <Tooltip>
                                      ...
                                      <TooltipContent className="max-w-[220px] bg-slate-900 text-white shadow-lg">
                                        ...
                                        <p className="text-xs leading-relaxed">
                                          Orders are usually completed faster
                                  ...
                                  {item.badges.lifetimeGuarantee && (
                                    <Tooltip>
                                      ...
                                      <TooltipContent className="max-w-[240px] bg-slate-900 text-white shadow-lg">
                                        ...
                                        <p className="text-xs leading-relaxed">
                                          We ensure your live link remains
                                          active. If any issue occurs, our team
                                          will resolve and restore the
                                          placement.
                                        </p>
                                      </TooltipContent>
                                    </Tooltip>
                          ...
                          <div className="grid grid-cols-1 gap-1.5">
                            <div className="flex items-center justify-between p-1 rounded bg-slate-50 border border-slate-100 max-w-[160px]">
                              <div className="flex items-center gap-1.5">
                                <span className="flex items-center justify-center size-3.5 rounded-sm bg-[#007ad8] text-white text-[8px] font-bold tracking-tighter">
                                  M
                                </span>
                            ...
                            <div className="flex items-center justify-between p-1 rounded bg-slate-50 border border-slate-100 max-w-[160px]">
                              <div className="flex items-center gap-1.5">
                                <span className="flex items-center justify-center size-3.5 rounded-sm bg-[#ff6300] text-white text-[8px] font-bold tracking-tighter">
                                  A
                                </span>
                                <span className="text-[10px] font-medium text-muted-foreground">
                                  DR
                            ...
                            <div className="flex items-center justify-between p-1 rounded bg-slate-50 border border-slate-100 max-w-[160px]">
                              <div className="flex items-center gap-1.5">
                                <span className="flex items-center justify-center size-3.5 rounded-sm bg-[#323642] text-white text-[8px] font-bold tracking-tighter">
                                  S
                                </span>
                                <span className="text-[10px] font-medium text-muted-foreground">
                                  Traffic
                          ...
                          <div className="flex flex-col items-end gap-1.5">
                            ...
                            {item.copywriting > 0 && (
                              <Tooltip>
                                ...
                                <TooltipContent
                                  side="left"
                                  className="bg-slate-900 text-white max-w-[180px]"
                                >
                                  <p className="text-xs font-semibold mb-0.5">
                                    Copywriting Fee
                                  </p>
                                  <p className="text-xs leading-relaxed">
                                    +${item.copywriting} if you need the article
                                    written by our team.
                                  </p>
                                </TooltipContent>
                              </Tooltip>
                            ...
                            {cartItems[item.id] ? (
                              <div className="inline-flex items-center rounded-md overflow-hidden shadow-sm">
                                ...
                                <button
                                  onClick={(e) => {
                                    e.stopPropagation();
                                    addItem(item); // This will correctly find the index and increment!
                                  }}
                              ...
                              <Button
                                ...
                                className={cn(
                                  ...
                                  addingItemId === item.id &&
                                    "scale-95 bg-green-500 hover:bg-green-500",
                                )}
                              >
                                {addingItemId === item.id ? (
                                  <>
                ...
                >
                  ...
                  {getPageNumbers().map((page, index) =>
                    page === "..." ? (
                      <span
                        key={`ellipsis-${index}`}
                        className="relative inline-flex items-center px-4 py-2 border border-border bg-white text-sm font-medium text-muted-foreground"
                      >
                        ...
                      </span>
                    ) : (
